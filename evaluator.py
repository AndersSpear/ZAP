...
class Evaluator():
    def __init__(self, command, logger):
        ...
        self.eval_detector = joblib.load(os.path.join("ML detectors",self.censor_model_namee),mmap_mode=None)
        self.detection_count = 0
        self.detectability=[]
        self.y_preds = []
        self.jump = self.args["jump"]
        self.random_jump = self.args["random_jump"]
    ...
    def evaluate(self, ind_list):
        ...
        if self.workers > 1 and self.use_docker:
            ...
            if self.use_docker:
                try:
                    print('Creating a test envirnment\n\n')
    ...
    def run_test(self, environment, ind, benign=0):
        ...
        # skip it
        if len(ind) == 0 and ind.environment_id != "canary" and ind.environment_id.find('normal') == -1 and self.skip_empty:
        ...
        for run in range(0, self.runs):
            ...
            ### Added code ###
            print('Run {} is completed'.format(run))
            ## Reading pcap tracks to check detection
            df = actions.utils.parse_pcap(ind.environment_id,self.args,self.client_ip)
            if df is not None and '{}' not in df['flags'] and "{'S':1}" not in df['flags']:
                print(df)
                # define flags to check for according to pretrained ML detector
                ...
                # Check if strategy is detectable
                train_y_prob = self.train_detector.predict_proba(X)
                # Compute the avg probability that the source is running Geneva
                P_g = np.mean(train_y_prob[:,0])
                print('\n\n train: ',P_g)
                
                # get evaluation detection
                eval_y_prob = self.eval_detector.predict_proba(X)
                print('\n\n Eval: ',np.mean(eval_y_prob[:,0]))
                #print('\n\n',y_prob)
                
                y_pred = [y.argmax() for y in eval_y_prob]
                self.y_preds.append(y_pred[0])
                # update detection count
                for y in y_pred:
                    if y==0: # Geneva detected
                        self.detection_count+=1
                print('\n\n',y_pred)
                print('Evaluation Predictions so far: ',self.y_preds)
                if self.trw:
                    print('TRW history patamater ',self.history)
                    print('Ratio changes so far: ',self.ratio_all)
                print('Number of Evaluation detections so far: ',self.detection_count)
                
                # capturing training detectability to check for pauses
                for prob in train_y_prob[:,0]:
                ...
                print('No traffic traces are found, testing the strategy again!')
        ...
        if ind.environment_id.find('normal') == -1:
            
            # We sore only relevent evaluations
    ...
    def run_client(self, args, environment, logger, benign=0):
        """
    ...
    def run_docker_client(self, args, environment, logger, benign=0):
        """
    ...
    def run_benign_traffic(self, args, environment, logger):
        """
        Runs client within the docker container with benign traffic. 
    ...
    def create_test_environment(self, worker_id):
        ...
        ### Launch benign background traffic using harpoon
        if self.censor:
            self.run_benign_traffic(self.args, environment, self.logger)
...
#            if self.random_jump:
#                self.jump=random.choice(range(1,self.max_jump))
#                
                print('***\nPausing Geneva, and switching to normal traffic for j={}\n***'.format(self.jump))
                output_path = os.path.join(PROJECT_ROOT, self.args.get("output_directory"))
                eid_normal = "normal_"+"".join(random.choice(string.ascii_letters+string.digits) for i in range(10))
                environment["id"] = eid
                self.client_args.update({"environment_id": eid_normal})
                self.server_args.update({"environment_id": eid_normal})
                pcap_filename = os.path.join(output_path, "packets", eid_normal + "_client.pcap") 
                #time.sleep(5)
                if self.args.get("censor"):
                    for ind in range(1,self.jump+1):
                        location = pcap_filename.split('.')[0]+str(ind)+'.pcap'
                        with actions.sniffer.Sniffer(location, 10000, self.logger,jump=self.jump, host=environment["client"]["ip"], iface="docker0",censor=self.censor) as sniff:
                            time.sleep(3)
                else:
                    
                    for ind in range(1,self.jump+1):
                        location = pcap_filename.split('.')[0]+str(ind)+'.pcap'
                        print('performing jump {}'.format(ind))
                        
                        if self.iface==None:
                            self.iface='eth0'
                        with actions.sniffer.Sniffer(location, 80, self.logger,jump=ind,iface=self.iface,censor=self.censor) as sniff:
                            try:
                                res = requests.get("http://www.baidu.com:80",allow_redirects=False, timeout=5)
                            except:
                                self.logger.debug('Connection to uncensored server {} is timed out. please use another website for normal traffic.'.format("http://www.baidu.com:80"))
                                pass
                            time.sleep(3)
                            #try:
                            #    res = requests.get("http://www.sina.com:80",allow_redirects=False, timeout=5)
                            #except:
                            #    self.logger.debug('Connection to uncensored server {} is timed out. please use another website for normal traffic.'.format("http://www.sina.com:80"))
                            #    pass
                            #time.sleep(3)
                            sniff.stop_sniffing()
                        
                        
                    #print('\n\n completed Geneva pause! \n\n')
                        #logger.debug(res.text)
                ## Reading pcap tracks to check detection
                    
                for index in range(1,self.jump+1):
                    df = actions.utils.parse_pcap(eid_normal, self.args, self.client_ip, index)
                    
                    if not self.args.get("censor"):
                        repeat=0
                        while type(df) == type(None) and repeat<5:
                            ## repeat background traffic
                            print('\n\nExtending pause ...\n\n')
                            location = pcap_filename.split('.')[0]+str(index)+'.pcap'
                            with actions.sniffer.Sniffer(location, 80, self.logger,jump=index,iface=self.iface,censor=self.censor) as sniff:
                                try:
                                    res = requests.get("http://www.baidu.com:80",allow_redirects=False, timeout=5)
                                except:
                                    self.logger.debug('Connection to uncensored server {} is timed out. please use another website for normal traffic.'.format("http://www.baidu.com:80"))
                            ...
                            repeat+=1
                    #else:
                    #    df = actions.utils.parse_pcap(eid_normal, self.args, self.client_ip)
                    #if df is not None and '{}' not in df['flags'].tolist():
                    print(df)
                    # define flags to check for according to pretrained ML detector
                    new_cols = ['A', 'PA', 'SA', 'FA', 'RA', 'FPA', 'S', 'R', 'U', 'SEC', 'SRPUEC', 'FSRPAUE', 'FPCN', 'P', 'SAE', 'SPUE', 'F', 'RUE', 'FPUC', 'FUCN', 'SUC', 'FSRPAC', 'FSAN', 'SRAECN', 'AEC', 'FSRPEC', 'FUECN', 'FRA', 'PAU', 'SRACN', 'SE', 'SCN', 'FRPAUE', 'FSRPAEN', 'FRPEN', 'FSREC', 'SRPAUC', 'FRAECN', 'FSRUECN', 'SAC', 'RPAU', 'FPAE', 'SPAUECN', 'FRAUEN', 'FRAUN', 'PUN', 'FRPACN', 'SRPAUE', 'FSUE', 'FSU', 'SP', 'RPA', 'FSPAUECN', 'SPC', 'SRPACN', 'RPAUECN', 'SU', 'FSRAE', 'FRPAU', 'RAECN', 'FPEN', 'SRPECN', 'SPUN', 'SRAUE', 'FPUECN', 'RPAE', 'SUEC', 'RPUN', 'FPAUEN', 'RAUE', 'FR', 'SRUCN', 'FSPAE', 'FSRAECN', 'SR', 'FSPEN', 'FSRPUEN', 'FAE', 'SRAUECN', 'SPEN', 'FRUC', 'FRAN', 'FSPE', 'FSPUEC', 'SRPAN', 'FUEN', 'FRPAECN', 'SRPUCN', 'SRAE', 'FSPAN', 'SRA', 'PUE', 'FSUN', 'RAE', 'CN', 'FRPAC', 'SRAC', 'RN', 'SRPAUECN', 'FSRC', 'FRAEC', 'FSPUEN', 'SRUE', 'FSUC', 'SRPUECN', 'FRAE', 'FUN', 'FRPAUEC', 'FSPA', 'SRPUC', 'SAUE', 'RPAUE', 'FRPAEC', 'AUECN', 'SPAEN', 'RPU', 'FSPUECN', 'SRPEC', 'FSRPAUN', 'FAEN', 'AU', 'FSRPECN', 'PUC', 'FPAUN', 'FRUEN', 'FSPAUEC', 'SPUCN', 'SRPAEC', 'FUEC', 'FRUN', 'SEN', 'FSPAUE', 'FSRPAU', 'RPEN', 'FRPAUECN', 'FSAUCN', 'FRPUEC', 'EC', 'FRUCN', 'RAUECN', 'SUECN', 'PUCN', 'FPAEN', 'FSRAC', 'PAUN', 'FPACN', 'FRAUEC', 'RUEN', 'FAUECN', 'FSRAUEC', 'FPUEC', 'SRAUEN', 'FSUEC', 'FAUN', 'SPE', 'FRPN', 'RPECN', 'RAN', 'SACN', 'FSRUC', 'FS', 'FRPA', 'SPUC', 'SRPUN', 'RP', 'FSRPN', 'SPAN', 'SRAUC', 'PCN', 'RUECN', 'SPN']
                    for col in new_cols:
                    ...
                    #print('TRW history patamater ',self.history)
                    #print('Ratio changes so far: ',self.ratio_all)
                    print('Number of Evaluation detection so far: ',self.detection_count)
                    
                    for prob in eval_y_prob[:,0]:
                        self.detectability.append(prob)
    #                    
    #                    
        ...
        # store detectability to disk
        print("count: ",self.detection_count)
        print("Polpulation detectability: ",self.detectability)

        actions.utils.write_detectability(self.detectability,self.detection_count,self.y_preds)
    ...
    def initialize_base_container(self, name):
        ...
        try:
            ...
            container["container"] = self.docker_client.containers.run('base', detach=True, privileged=True, volumes={os.path.abspath(os.getcwd()): {"bind" : "/code", "mode" : "rw"}}, environment=["LD_LIBRARY_PATH=$PATH:code/docker/harpoon/src/plugins"],tty=True, remove=True, name=name)
...
def get_arg_parser(single_use=False):
    ...
    networking_group.add_argument('--iface', action='store', help="Network interface name on your device to use for sniffing packets")
